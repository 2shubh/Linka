...
import { fetchConnections } from '../features/connections/connectioSlice'
    ...
    return (
      <div className='min-h-screen bg-slate-50'>
          ...
          rounded-md p-1 bg-white shadow-sm'>
            ...
            {
              dataArray.map((tab)=>(
              <button onClick={()=>setcurrentTab(tab.label)} key={tab.label} className={`flex items-center cursor-pointer px-3 py-1 text-sm
                rounded-md transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black '
                  : 'text-gray-500 hover:text-black'}`}>
                    ...
                    {tab.value?.length !== undefined  && (
                      ...
                      px-2 py-0.5 rounded-full'> {tab.value?.length} </span>
                    )}
                    </button>
              ))
            }
                    ...
                    <div className='flex max-sm:flex-col gap-2 mt-4'>
                      {
                        <button onClick={()=>navigate(`/profile/${user._id}`)}
                         ...
                         from-indigo-500 to-purple-700 hover:from-indigo-600
