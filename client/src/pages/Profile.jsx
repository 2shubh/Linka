...
console.log("currentUser:", currentUser);
 ...
 //Debug
 console.log("Profile Info Props:", { user, profileId, posts });
            ...
            {["posts" , "media" , "likes"].map((tab)=>(
              ...
              onClick={()=>setActiveTab(tab)}
               className={`flex-1  px-4 py-2 text-sm font-medium 
                ...
                ${activeTab == tab ? "bg-indigo-600 text-white"
            ...
            <div className='flex flex-wrap mt-6 max-w-6xl'>
             {
              posts.filter((post)=>post.image_urls.length > 0).map((post)=>(
                ...
                {post.image_urls.map((image,index)=>(
                  <Link target='__blank' to={image} key={index} className='relative group'>

                    <img src={image} key={index} className='w-64 aspect-video
                    object-cover' alt='' />
                    <p className='absolute bottom-0 right-0 text-xs
                    p-1 px-3 backdrop-blur-xl text-white opacity-0
