...
//inngest user function to update
  ...
  async({event , step})=>{
     ...
     await step.run('send-connection-request-mail') , async()=>{
      const connection = await Connection.findById(connectionId).populate(
        ...
        const body = `<div style="font-family:Arial, sans-serif;
        padding:20px;">
          <h2>Hi ${connection.to.user_id.full_name}, </h2>
...
const sendNotificationOfUnseenMessages = inngest.createFunction(
  ...
  async({step})=>{
    ...
    for(const userId in unseenCount){
      ...
      const body = 
    `<div style = "font-family:Arial, snas-serif ; padding: 20px;">
