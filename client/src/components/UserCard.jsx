...
const UserCard = ({user}) => {
    ...
    const handleFollow = async() =>{
      try{
        const {data} = await api.post('/api/user/follow',
          {id:user._id},{headers:{Authorization: `Bearer ${await getToken()}`}}
        )
        if(data.success){
  ...
  return (
    <div key={user._id} className='p-4 pt-6 flex flex-col justify-between w-72 
     ...
     {/* Follow Button */}
     <button disabled={currentUser?.following.includes(user._id)}
     onClick={handleFollow} className='w-full py-2 rounded-md flex justify-center 
     ...
     justify-center w-16 border text-slate-500 rounded-md cursor-pointer 
