    ...
    const fetchRecentMessages =async () =>{
        try{
            ...
            if(data.success){
                ...
                //group message by sender and get the latets message for
                const groupedMessages = data.messages.reduce((acc,message)=>{
                    const senderId = message.from_user_id._id;
                    if(!acc[senderId] || new Date(message.createdAt) > new 
                    Date(acc[senderId].createdAt)){
            ...
            const sortedMesaages = Object.values(groupedMessages).sort((a,b)=>{
                new Date(b.createdAt) - new Date(a.createdAt)
            })
            setMesaages(sortedMesaages);
        }else{
